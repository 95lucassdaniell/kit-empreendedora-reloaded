  ...
  return <section className="bg-gray-50 py-[50px]">
      <div className="container mx-auto px-6">
        ...
        <div className="hidden md:block max-w-4xl mx-auto bg-white rounded-2xl shadow-2xl overflow-hidden">
          ...
          {comparisons.map((comparison, index) => <div key={index} className="grid md:grid-cols-2 border-b border-gray-100 last:border-b-0">
        ...
        <div className="md:hidden space-y-8">
          ...
          <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
            ...
            {comparisons.map((comparison, index) => <div key={`without-${index}`} className="p-6 bg-red-50 flex items-center gap-4 border-b border-gray-100 last:border-b-0">
                <span className="text-red-500 text-xl">❌</span>
                <span className="text-gray-700">{comparison.without}</span>
              </div>)}
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
            ...
            {comparisons.map((comparison, index) => <div key={`with-${index}`} className="p-6 bg-green-50 flex items-center gap-4 border-b border-gray-100 last:border-b-0">
                <span className="text-green-500 text-xl">✅</span>
                <span className="text-gray-700">{comparison.with}</span>
              </div>)}
          </div>
